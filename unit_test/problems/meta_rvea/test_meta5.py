...
from evox.core import Algorithm, Mutable, Parameter, jit_class, trace_impl, debug_print
...
class OuterPSO(Algorithm):
    ...
    * `step`: Performs a single optimization step using Particle Swarm Optimization (PSO), updating local best positions and fitness values, and adjusting velocity and positions based on inertia, cognitive, and social components.
    ...
    def __init__(
        ...
        self.local_best_location = Mutable(population, device=device)
    ...
    def step(self):
        ...
        """
        self.gen = self.gen + torch.tensor(1, device=self.gen.device)
...
class InnerRVEAa(Algorithm):
    ...
    def __init__(
        ...
        self.pop = Mutable(population, device=self.device)
        self.fit = Mutable(torch.empty((self.pop_size * 2, self.n_objs), device=device).fill_(torch.inf), device=self.device)
        self.reference_vector = Mutable(v, device=self.device)
        self.init_v = v0
    ...
    def init_step(self):
        ...
        """
        self.reference_vector = torch.cat([torch.as_tensor(self.init_v, device=self.device), torch.as_tensor(self.ref_vec_init, device=self.device)], dim=0)
    ...
    def _trace_update_pop_and_rv(self, survivor: torch.Tensor, survivor_fit: torch.Tensor):
        ...
        state2 = if_else2.cond(state2, self.gen - self.max_gen == 0, survivor, survivor_fit)

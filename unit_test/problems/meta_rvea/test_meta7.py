# Outer algorithm: A variant of PSO, size of population and velocity: pop_size * inner_pop_size * n_objs
...
# Result of n_objs = 3 (IGD):
# Result of n_objs = 5 (IGD):
# Result of n_objs = 10 (IGD):
# Result of n_objs = 10 and outer iterations = 1000 (IGD):
# Result of n_objs = 15 (IGD):
...
class OuterPSO(Algorithm):
    ...
    * `step`: Performs a single optimization step using Particle Swarm Optimization (PSO), updating local best positions and fitness values, and adjusting velocity and positions based on inertia, cognitive, and social components.
    ...
    def __init__(
        ...
        length = ub - lb
        population = torch.rand(self.pop_size, self.inner_pop_size, self.n_objs, device=device)
        population = length.view(self.inner_pop_size, self.n_objs) * population + lb.view(self.inner_pop_size, self.n_objs)
    ...
    def step(self):
        ...
        population = self.population.view(self.pop_size, -1)
        fitness = self.evaluate(population)
        compare = self.local_best_fitness > fitness
        self.local_best_location = torch.where(compare[:, None], population, self.local_best_location)
        ...
        self.global_best_location, self.global_best_fitness = min_by(
            [self.global_best_location.unsqueeze(0), population],
        ...
        velocity = (
            ...
            + self.phi_p * rp * (self.local_best_location - population)
            + self.phi_g * rg * (self.global_best_location - population)
        )
        ...
        if self.gen > self.max_gen / 2:
            ...
            distance_instance = self.global_best_location - population
        ...
        population = population / torch.linalg.vector_norm(population, dim=-1, keepdim=True)
        self.population = clamp(
            population, self.lb.view(self.inner_pop_size, self.n_objs), self.ub.view(self.inner_pop_size, self.n_objs)
        )

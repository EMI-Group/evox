"""Smoke test for the functional ETL port of SLPSOUS (no torch)."""
...
from evox_etl.algorithms.so.pso_variants import sl_pso_us


def test_sl_pso_us_smoke():
    """3 generations on 20-D Sphere: correct shapes and finite fitnesses."""
    cfg = sl_pso_us.make_sl_pso_us(
        pop_size=100,
        lb=np.full(20, -10.0, np.float32),
        ub=np.full(20, 10.0, np.float32),
    )
    state = run_generations(sl_pso_us, cfg, SphereConfig(20), n_gens=3, seed=0)

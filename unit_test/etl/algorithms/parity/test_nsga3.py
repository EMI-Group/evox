"""Parity test: evox_etl NSGA3 vs the torch evox reference on DTLZ1.
...
- etl side: the functional init/ask/tell port driven by
  ...
  minimum of the final population (NSGA-III is elitist — the final
...
def test_nsga3_dtlz1_parity():
    cfg = nsga3.make_nsga3(
        pop_size=POP_SIZE,
        n_objs=N_OBJ,
        lb=np.zeros(DIM, np.float32),
        ub=np.ones(DIM, np.float32),
    )
    state = run_generations(nsga3, cfg, DTLZ1Config(DIM, N_OBJ), n_gens=N_GENS, seed=SEED)
    etl_best = np.min(np.asarray(state.fit.numpy()), axis=0)

    torch_best, torch_final = torch_reference(evox.algorithms.NSGA3)

    print(f"NSGA3 parity (DTLZ1 d={DIM} m={N_OBJ}, {N_GENS} gens, seed {SEED}):")

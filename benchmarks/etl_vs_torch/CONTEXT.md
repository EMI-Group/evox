...
- **`etl-xla-cuda` harness mapping is stale and costs 5-17x:** `bench_common.etl_backend_spec` returns `("xla", None)` (`bench_common.py:159-160`) and the docstring at 146-151 claims the xla adapter is CPU-only — no longer true (etl HEAD f2f50a7 supports device-resident `Device("cuda", N)` executables). `None` makes the workflow CPU-kind (`workflow.py:45-48`), so the xla adapter host-stages EVERY input every step (`.numpy()` + `buffer_from_host` per call, `xla.py:639/660-669`) and round-trips outputs. Measured PSO/Sphere on a healthy GPU: 6.23 / 15.32 ms/step @1000x50 / @10000x100 — reproduces the committed 5.73 / 17.0.

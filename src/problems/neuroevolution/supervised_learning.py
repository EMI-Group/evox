...
__supervised_data__: Dict[
    int, Dict[
        str, DataLoader | Iterable | Iterator | Tuple
    ]
] = None
...
class SupervisedLearningProblem(Problem):
    ...
    def __init__(self, 
        ...
        inference_model.load_state_dict = types.MethodType(
            assign_load_state_dict, inference_model
        )
        ...
        self._jit_state_criterion = jit(state_criterion,
            trace = True, lazy = False, 
            example_inputs = (
                criterion_init_state, dummy_single_logits, dummy_labels,
            ),
        )
        ...
        sample_param_key, sample_state_key = next(
            iter(self.param_to_state_key_map.items())
        )
    ...
    def _data_loader_has_next(self) -> bool:
        global __supervised_data__
        return (
            __supervised_data__[self._hash_id_]["data_next_cache"] is not None
        )

    def _vmap_evaluate(self, 
        ...
        criterion_state = {
            key: value.clone() 
            for key, value in self.vmap_criterion_init_state.items()
        }
    ...
    def _single_evaluate(self, 
        ...
        criterion_state = {
            key: value.clone() 
            for key, value in self.criterion_init_state.items()
        }
    ...
    def evaluate(self, pop_params: Dict[str, nn.Parameter]) -> torch.Tensor:
        ...
        if (
            pop_params_value.ndim > self._sample_param_ndim 
            and pop_params_value.size(0) != 1
        ):
            pop_fitness = self._vmap_evaluate(
                pop_params, pop_params_value.size(0), pop_params_value.device,
            )
        else: 
            pop_fitness = self._single_evaluate(
                pop_params, pop_params_value.device,
            )
        return pop_fitness 

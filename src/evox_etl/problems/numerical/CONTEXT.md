...
- Config dataclasses holding numpy arrays (basic.py shift/affine) need NO registration on the installed etl (master f2f50a7, numpy backend): ndarray is accepted as a static trace leaf, so the former zero-child `etl.register_pytree_node` loop is gone (do not re-add it — a registered config becomes an opaque node that BYPASSES the run-time static-value equality check: a different config value at `etl.run` is accepted silently → wrong-answer drift, whereas plain-leaf ndarray fields raise TraceError on mismatch). Plain int-field dataclasses (dtlz/cec2022) work as static args directly.
...
## Config design (current state)
- Problems keep their validation-only `__post_init__`s as the direct-construction API: basic.py's ten configs each call `_validate_shift_affine` (shift vector / square affine, same dim), cec2022.py's `CEC2022` checks the dimension set.
- The validators and their messages are frozen — asserted by unit tests (`unit_test/etl/problems/test_basic.py` ~L183-195, `test_cec2022.py` ~L83-90).
- The zero-child `etl.register_pytree_node` loop (former basic.py tail) is DELETED: ndarray statics are legal on the installed etl, and unregistered plain-leaf configs keep the run-time static-value check (drifted config at `etl.run` raises TraceError).
- Problems do NOT use the algorithms' `make_*` constructor pattern — that policy lives in DESIGN.md §4.

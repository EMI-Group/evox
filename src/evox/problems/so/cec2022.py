...
class CEC2022(Problem):
    ...
    def cec2022_f9(self, x: torch.Tensor) -> torch.Tensor:
        ...
        fit = [
            self.rosenbrock_func( self.sr_func_rate( x, 2.048e-2, True, True , self.OShift[:,0*nx:1*nx], self.M[:,0*nx:1*nx] ) ) * 10000 / 1e4,
            self.ellips_func    ( self.sr_func_rate( x, 1.0     , True, True , self.OShift[:,1*nx:2*nx], self.M[:,1*nx:2*nx] ) ) * 10000 / 1e10,
            self.bent_cigar_func( self.sr_func_rate( x, 1.0     , True, True , self.OShift[:,2*nx:3*nx], self.M[:,2*nx:3*nx] ) ) * 10000 / 1e30,
            self.discus_func    ( self.sr_func_rate( x, 1.0     , True, True , self.OShift[:,3*nx:4*nx], self.M[:,3*nx:4*nx] ) ) * 10000 / 1e10,
            self.ellips_func    ( self.sr_func_rate( x, 1.0     , True, False, self.OShift[:,4*nx:5*nx], self.M[:,4*nx:5*nx] ) ) * 10000 / 1e10,
        ]
    ...
    def cec2022_f10(self, x: torch.Tensor) -> torch.Tensor:
        ...
        bias = [0, 200, 100]
        fit = [
            self.schwefel_func ( self.sr_func_rate( x, 10.0   , True, False, self.OShift[:,0*nx:1*nx], self.M[:,0*nx:1*nx] ) ) * 1.0, 
            self.rastrigin_func( self.sr_func_rate( x, 5.12e-2, True, True , self.OShift[:,1*nx:2*nx], self.M[:,1*nx:2*nx] ) ) * 1.0,
            self.hgbat_func    ( self.sr_func_rate( x, 5.00e-2, True, True , self.OShift[:,2*nx:3*nx], self.M[:,2*nx:3*nx] ) ) * 1.0
        ]
        f = self.cf_cal(x, fit, delta, bias)
        return f + 2400.0
    ...
    def cec2022_f11(self, x: torch.Tensor) -> torch.Tensor:
        ...
        fit = [
            self.escaffer6_func ( self.sr_func_rate( x, 1.0     , True, True, self.OShift[:,0*nx:1*nx], self.M[:,0*nx:1*nx] ) ) * 10000 / 2e7,
            self.schwefel_func  ( self.sr_func_rate( x, 10.0    , True, True, self.OShift[:,1*nx:2*nx], self.M[:,1*nx:2*nx] ) ) * 1.0,
            self.griewank_func  ( self.sr_func_rate( x, 6.0     , True, True, self.OShift[:,2*nx:3*nx], self.M[:,2*nx:3*nx] ) ) * 1000 / 100,
            self.rosenbrock_func( self.sr_func_rate( x, 2.048e-2, True, True, self.OShift[:,3*nx:4*nx], self.M[:,3*nx:4*nx] ) ) * 1.0,
            self.rastrigin_func ( self.sr_func_rate( x, 5.12e-2 , True, True, self.OShift[:,4*nx:5*nx], self.M[:,4*nx:5*nx] ) ) * 10000 / 1e3,
        ]
        f = self.cf_cal(x, fit, delta, bias)
        return f + 2600.0
    ...
    def cec2022_f12(self, x: torch.Tensor) -> torch.Tensor:
        ...
        fit = [
            self.hgbat_func     ( self.sr_func_rate( x, 5.00e-2, True, True, self.OShift[:,0*nx:1*nx], self.M[:,0*nx:1*nx] ) ) * 10000 / 1000,
            self.rastrigin_func ( self.sr_func_rate( x, 5.12e-2, True, True, self.OShift[:,1*nx:2*nx], self.M[:,1*nx:2*nx] ) ) * 10000 / 1e3,
            self.schwefel_func  ( self.sr_func_rate( x, 10.0   , True, True, self.OShift[:,2*nx:3*nx], self.M[:,2*nx:3*nx] ) ) * 10000 / 4e3,
            self.bent_cigar_func( self.sr_func_rate( x, 1.0    , True, True, self.OShift[:,3*nx:4*nx], self.M[:,3*nx:4*nx] ) ) * 10000 / 1e30,
            self.ellips_func    ( self.sr_func_rate( x, 1.0    , True, True, self.OShift[:,4*nx:5*nx], self.M[:,4*nx:5*nx] ) ) * 10000 / 1e10,
            self.escaffer6_func ( self.sr_func_rate( x, 1.0    , True, True, self.OShift[:,5*nx:6*nx], self.M[:,5*nx:6*nx] ) ) * 10000 / 2e7,
        ]
        f = self.cf_cal(x, fit, delta, bias)
        return f + 2700.0

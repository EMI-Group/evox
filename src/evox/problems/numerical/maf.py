...
class MAF6(MAF):
    ...
    def evaluate(self, X: torch.Tensor):
        ...
        f = (
            ...
            * torch.cat([torch.ones((n, 1)), torch.sin(torch.flip(X[:, : m - 1], [1]) * torch.pi / 2)], dim=1)
        )
        return f
...
class MAF8(MAF):
    ...
    @torch.jit.ignore
    def _cal_pf(self):
        temp = torch.linspace(-1, 1, steps=int(torch.sqrt(torch.tensor(self.ref_num * self.m, device=self.device))), device=self.device)
    ...
    def _point_in_polygon(self, polygon, point):
        ...
        is_intersect = torch.cat([self._ray_intersect_segment(point, polygon[i], seg_term[i]) for i in range(polygon.size(0))], dim=0)
...
class MAF12(MAF):
    ...
    def evaluate(self, X: torch.Tensor):
        ...
        t3[:, m - 1] = (torch.sum(t2[:, m - 1 :], dim=1) + SUM * 2) / torch.tensor(L / 2, device=X.device) / (1 + 2 * L - 2 * torch.tensor(L / 2, device=X.device))
...
class MAF14(MAF):
    ...
    def _modify_X(self, X: torch.Tensor):
        X[:, self.m - 1 :] = (
            (1 + torch.arange(self.m, self.d + 1, device=X.device) / self.d) * X[:, self.m - 1 :] - (X[:, 0] * 10).unsqueeze(-1)
        )
        return X
...
class MAF15(MAF14):
    ...
    def _modify_X(self, X: torch.Tensor):
        X[:, self.m - 1 :] = (
            1 + torch.cos(torch.arange(self.m, self.d + 1, device=X.device) / self.d * torch.pi / 2)

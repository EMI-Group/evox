...
- **`vector_metric_problem.py`** → `VectorMetricProblem(Problem)` — Flat-vector virtual-metric experiment. Treats all parameters as a single flat vector of length `n_params` (no model forwarding) and computes a normalized metric directly via the fused `virtual_reduce_metric` kernel. Supports both the virtual `(center, seeds, sigma)` path (noise never materialized, scales to `pop_size` 16384+) and a naive/materialized 2D-population path `population.abs().mean(dim=-1)`. Does NOT support HPO wrapper.

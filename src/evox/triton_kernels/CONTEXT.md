...
| Virtual noise utilities | `kernels/virtual_noise.py` | Virtual (never-materialized) noise fused into matmul/reduction. `virtual_perturbed_linear` (forward: `Y[i]=X@(W+sigma*N_i)^T+...`), `virtual_weight_gradient`/`virtual_bias_gradient` (regenerate the *same* noise for gradient estimates), `virtual_reduce_metric` (per-individual `mean_k(|center[k]+sigma*noise[i,k]|)` reduction, never materializing the noise; one program per individual), `compute_offsets` (cumulative flat-element offsets). Uses a **fast centered-uniform RNG** (`tl.rand(seed,offsets)-0.5` on Triton; splitmix64-hash uniform centered to `[-0.5,0.5)` on the PyTorch fallback) — a cheap approximation since RNG distributional quality is irrelevant for this forward-only demo (only determinism matters). Forward↔gradient regenerate identical noise; Triton and CPU paths need not match each other |

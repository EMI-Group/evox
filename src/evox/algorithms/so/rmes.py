...
@evox.jit_class
class RMES(CMAES):
    def __init__(self, center_init, init_stdev, pop_size=None, recombination_weights=None, cm=1, elite_ratio=0.5, memory_size=10, mean_decay=0, sparse_threshold=0.01,t_uncorr=10):
    ...
    def setup(self, key):
        ...
        return base_state.update(P=P, t_gap=t_gap, s_rank_rate=s_rank_rate, fitness_archive=fitness_archive, p_sigma=p_sigma)
    ...
    def tell(self, state, fitness):
        ...
        p_sigma_new = (1 - self.c_sigma) * state.p_sigma + jnp.sqrt(self.c_sigma * (2 - self.c_sigma) * self.mueff) * (mean - state.mean) / state.sigma
        P, t_gap = self._update_P_matrix(state.P, state.p_sigma, state.t_gap, state.count_iter)
        s_rank_rate = self._rank_success_rule(fitness, state.fitness_archive)

        return new_state.update(mean=mean, p_sigma=p_sigma_new, P=P, t_gap=t_gap, s_rank_rate=s_rank_rate, fitness_archive=fitness)
    ...
    def _update_P_matrix(self, P, p_sigma, t_gap, gen_counter):
        ...
        def body_fun(i, vals):
            ...
            P_c2_new = jax.lax.cond(replace_bool, lambda _: P_c2.at[:, i].set(P_c2[:, i + 1]), lambda _: P_c2, None)
            t_gap_c2_new = jax.lax.cond(replace_bool, lambda _: t_gap_c2.at[i].set(t_gap_c2[i + 1]), lambda _: t_gap_c2, None)
        ...
        t_gap1 = jax.lax.cond(push_replace, lambda _: t_gap_c1, lambda _: t_gap_c2, None)
    ...
    def _sparse_plus_low_rank_update(self, y, pc):
        ...
        # Sparse update
        sparse_update = jnp.where(jnp.abs(jnp.outer(y_aggregated, pc)) > self.sparse_threshold, jnp.outer(y_aggregated, pc), 0)
    ...
    def _rank_success_rule(self, fitness, fitness_archive):
        ...
        concat_all = jnp.vstack([jnp.expand_dims(fitness, 1), jnp.expand_dims(fitness_archive, 1)])
        ...
        new_s_rank_rate = (1 - self.c_s) * self.s_rank_rate + self.c_s * (q - self.q_star)

...
def row_argsort(
    ...
    """
    y = jnp.argsort(x, axis=-1, kind='stable') # sort each row of x and get a N*M index array
    return y
...
def test_method():
    ...
    values = jax.random.uniform(key, shape=(3, 2))
    dm = jax.jit(get_distance_matrix)
    matrix = dm(values)
    print(values)
    print(matrix)
    sorted = jax.jit(row_argsort)(matrix)
    print(sorted)

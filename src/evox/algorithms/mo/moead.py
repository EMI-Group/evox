...
class MOEAD(ex.Algorithm):
    ...
    def _ask_normal(self, state):
        ...
        return (
            state.update(next_generation=next_generation, parent=parent, key=key),
            next_generation,
        )
        ...
        for i in range(self.pop_size):
            ...
            for j in range(self.T):
                population = jax.lax.cond(
                    g_old[j] >= g_new[j],
                    lambda: population.at[ind_p[j]].set(offspring[j]),
                    lambda: population.at[ind_p[j]].set(temp_pop[ind_p[j]]),
                )
                pop_obj = jax.lax.cond(
                    g_old[j] >= g_new[j],
                    lambda: pop_obj.at[ind_p[j]].set(obj[j]),
                    lambda: pop_obj.at[ind_p[j]].set(temp_obj[ind_p[j]]),
                )

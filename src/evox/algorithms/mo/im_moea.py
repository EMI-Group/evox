...
from evox.operators.selection import NonDominate, non_dominate
...
class IMMOEA(Algorithm):
    ...
    # select next generation, but we need add clustering, then select
    def tell(self, state, fitness):
        merged_pop = jnp.concatenate([state.population, state.next_generation], axis=0)
        merged_fitness = jnp.concatenate([state.fitness, fitness], axis=0)
        ...
        jax.debug.print("class_num:{}",class_num)
        all_sub_pop = []
        all_sub_pop_fit = []
        ...
        def handle_small_class(mask):
            ...
            # jax.debug.print("small_mask.sum: {}", jnp.sum(mask))
            # jax.debug.print("sub_indices.shape:{}", sub_indices.shape)
            # jax.debug.print("sub_pop_fitness:{}",sub_pop_fitness)
        ...
        def handle_large_class(mask):
            ...
            sub_pop, sub_pop_fitness, _ = self.nsgaii_selection(state, merged_pop, merged_fitness, mask, class_num[i])
            return sub_pop, sub_pop_fitness

        for i in range(self.k):
            mask = partition == i
            # jax.debug.print("i:{}", i)
            # jax.debug.print("mask.sum:{}", jnp.sum(mask))
            # jax.debug.print("class_num[i]:{}",class_num[i])
            sub_pop, sub_pop_fitness = lax.cond(class_num[i] <= std_size,
                                                handle_small_class,
                                                handle_large_class,
                                                operand=mask)

            # jax.debug.print("sub_pop[class_num[i]-5:class_num[i]+5,:]: {}", sub_pop[:40,:])
        ...
        # jax.debug.print("next_fitness: {}", next_fitness)
    ...
    def get_class_num(self, state, partition):
        ...
        for i in range(self.k):
            ...
            allocation += lax.cond(sub_pop_num <= std_size, lambda : sub_pop_num, lambda : std_size)
            class_mask = class_mask.at[i].set(lax.cond(sub_pop_num <= std_size, lambda : 0, lambda : 1))
        all_remain = 2*self.pop_size - allocation
        cur_remain = self.pop_size - allocation
        for i in range(self.k):
            num = lax.cond(class_mask[i], lambda : std_size+jnp.ceil((class_num[i]-std_size)*cur_remain/all_remain), lambda : class_num[i])
        ...
        all_remain, class_num = lax.while_loop(lambda x:  x[0]<0, loop_body, (fin_remain, class_num))
    ...
    def nsgaii_selection(self, state, merged_pop, merged_fitness, mask, sub_pop_size):
        ...
        population, fitness = non_dominate(population=merged_pop,fitness=fitness,topk=self.pop_size)
        # jax.debug.print("population.shape:{}", population.shape)
        mask_ = jnp.arange(self.pop_size) < sub_pop_size
        fitness = jnp.where(mask_[:, jnp.newaxis], fitness, jnp.inf)
        # jax.debug.print("survivor_fitness.shape:{}", survivor_fitness.shape)
    ...
    def _gp_offspring_(self, state, population, mask, fitness):
        ...
        def normal_fun(x_key):
            ...
            final_pop_indices = jnp.where(pop_indices % jnp.floor(N/n) == 0, pop_indices, -1)
            ...
            shuffled_din_indices_group = jax.vmap(lambda i: get_dim_indices(i, shuffled_dim_indices))(arr)
            ...
            def gp_body(m, shuffled_indices_group, _keys, new_pop, new_fit):
                ...
                parents = jnp.where(pop_indices <= jnp.floor(n/self.n_objs), permutation, -1)
                _mask = pop_indices == parents
                sub_off, sub_fit = random_fill(matrix=new_pop, matrix2=new_fit, key=key_arr[1], mask=_mask)
                ...
                inputs = jnp.where(final_pop_indices >= 0, inputs, jnp.inf)[:, jnp.newaxis]
                _sub_fit = lax.dynamic_slice(sub_fit, (0, m), (sub_fit.shape[0], 1))

                # get the offspring of the GP model
                def get_off(i, dim_indices, _keys, model, sub_off, sub_fit):
                    _sub_pop = lax.dynamic_slice(sub_off, (0,dim_indices[i]), (sub_fit.shape[0], 1))
                ...
                res = jax.vmap(lambda i: get_off(i, dim_indices, _keys, model, sub_off, sub_fit))(
                    jnp.arange(self.l))
                return res

            x_keys = jax.random.split(x_key, num=self.n_objs)
            # sub_off is a list of offspring of each objective. M*L*N matrix
            sub_off = jax.vmap(lambda i: gp_body(i, shuffled_din_indices_group, x_keys, new_pop, new_fit))(arr)
        ...
        final_pop = lax.cond(n >= 2 * self.n_objs, normal_fun, lambda x: population, x_key)

...
def stochastic_ranking_selection(key, pc, I1, I2):
    ...
    def body_fun(vals):
        ...
        def in_body(j, vals):
            ...
            def true_fun(rank, swapdone):
                ...
                rank, swapdone = jax.lax.cond(I1[rank[j]] < I1[rank[j + 1]], in_true, in_false, rank, swapdone)
            ...
            def false_fun(rank, swapdone):
                ...
                def in_false(rank, swapdone):
                    return rank, swapdone
                rank, swapdone = jax.lax.cond(I2[rank[j]] < I2[rank[j + 1]], in_true, in_false, rank, swapdone)
                return rank, swapdone
    ...
    rank, s, a = jax.lax.while_loop(lambda vals: (vals[2] < n_half) & (vals[1]), body_fun, (rank, swapdone, i))
    return rank
...
class SRA(Algorithm):
    ...
    def _ask_normal(self, state):
        ...
        mating_pool = jax.random.randint(sel_key, (self.pop_size * 2, ), 0, self.pop_size)
    ...
    def _tell_normal(self, state, fitness):
        ...
        population, pop_obj = environmental_selection(env_key, merged_pop, merged_fitness, self.pop_size, pc)

...
class SLPSOUS(Algorithm):
    """The basic Particle Swarm Optimization Social Learning PSO Using Uniform Sampling for Demonstator Choice (SLPSOUS) algorithm.
    ...
    * `step`: Performs a single optimization step using Particle Swarm Optimization (SLPSOGS), updating local best positions and fitness values, and adjusting velocity and positions based on inertia, cognitive, and social components.
    ...
    def _set_random(self,q):
        ...
        uniform_distribution = torch.rand(
            self.pop_size, device=self.population.device) * (self.pop_size + 1 - q) + q
        return r1, r2, r3, uniform_distribution

    @trace_impl(_set_random)
    def _trace_set_random(self,q):
        ...
        uniform_distribution = batched_random(

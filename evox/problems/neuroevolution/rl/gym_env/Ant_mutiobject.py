...
class Worker:
    def __init__(self, env_options, num_env, policy=None):
    ...
    def step(self, actions):
        for i, (env, action) in enumerate(zip(self.envs, actions)):
            ...
            if not (self.terminated[i] or self.truncated[i]):
                ...
                survive, forward_distance, ctrl_cost = self.infos[i]["reward_survive"], self.infos[i]["reward_forward"], self.infos[i]["reward_ctrl"]
                
                self.total_rewards[i] += reward
                self.total_survive[i] += survive
                self.total_forward[i] += forward_distance
                self.total_ctrl[i] += ctrl_cost
    ...
    def reset(self, seeds):
        ...
        self.total_survive = [0 for _ in range(self.num_env)]
        self.total_forward = [0 for _ in range(self.num_env)]
        self.total_ctrl = [0 for _ in range(self.num_env)]
    ...
    def rollout(self, seeds, subpop, cap_episode_length):
        ...
        return self.total_rewards, self.total_survive, self.total_forward, self.total_ctrl, self.episode_length
...
class Controller:
    ...
    def _evaluate(self, seeds, pop, cap_episode_length):
        ...
        rewards, survive, forward, control, episode_length = zip(*ray.get(rollout_future))
        return np.array(rewards).reshape(-1), np.array(survive).reshape(-1), np.array(forward).reshape(-1), np.array(control).reshape(-1), np.array(episode_length).reshape(-1)
...
class Gym(Problem):
    ...
    def evaluate(self, state, pop):
        ...
        rewards, survive, forward, control, episode_length = ray.get(
        ...
        fitness = zip(-survive, -forward, -control) # reward = survive + forward + control * lambda
        fitness = jnp.asarray(list(fitness))
    
        if self.cap_episode:
            state = self.cap_episode.update(state, episode_length)

        # i am familiar to this env, so i dont need it

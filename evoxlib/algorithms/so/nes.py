...
@exl.jit_class
class SeparableNES(exl.Algorithm):
    def __init__(self, init_mean, init_var, pop_size=None, recombination_weights=None, learning_rate_mean=None, learning_rate_var=None):
        """
        See [link](https://arxiv.org/pdf/1106.4487.pdf) for default parameters
        """
        assert(pop_size > 0)
        ...
        self.dim = init_mean.shape[0]
        self.init_mean = init_mean
        assert(init_var.shape == (self.dim,))
    ...
    def new_pop(self, key, mean, sigma):
        key, sample_key = jax.random.split(key)
        zero_mean_pop = jax.random.normal(sample_key, shape=(self.pop_size, self.dim))
        population = lax.map(lambda p: mean + sigma * p, zero_mean_pop)
        return population, zero_mean_pop, key
    ...
    def setup(self, key):
        ...
        population, zero_mean_pop, key = self.new_pop(key, mean, sigma)
        return exl.State(population=population, zero_mean_pop=zero_mean_pop, mean=mean, sigma=sigma, key=key)
    ...
    def tell(self, state, fitness):
        ...
        grad_μ = jnp.sum(lax.map(lambda u, s: u * s, zip(self.weight, zero_mean_pop)), axis=1)
        grad_σ = jnp.sum(lax.map(lambda u, s: u * (s * s - 1), zip(self.weight, zero_mean_pop)), axis=1)
